...
exports.assessHtmlPage = assessHtmlPage;
exports.assessStructuredPage = assessStructuredPage;
exports.describeNextActionHint = describeNextActionHint;
function assessHtmlPage(html, readableText) {
...
function assessStructuredPage(data) {
...
function buildDiagnostics(html, readableText) {
    return {
        ...
        shellMarkersDetected: /__NEXT_DATA__|__NUXT__|data-sveltekit|\/__data\.json\b|webpackJsonp|window\.__|hydration|astro-|elementorFrontendConfig|wix-code-sdk|squarespace/i.test(html)
    };
}
function collectSignals(html, readableText) {
...
function classifyAuthwallText(value) {
    if (/(sign in to view|sign in to continue|join now|join to view|log in to continue|create account|sign up to continue|member only|members only)/i.test(value)) {
        return "Login or signup wall markers matched the returned page";
    }
    return undefined;
}
function classifyChallengeText(value) {
    if (/(verify you are human|security verification|captcha|checking your browser|enable javascript and cookies|just a moment|one more step)/i.test(value)) {
        return "Challenge markers matched the returned page";
    }
    return undefined;
}
function classifyBlockedText(value) {
    if (/(access denied|request blocked|blocked request|temporarily unavailable|you have been blocked|forbidden)/i.test(value)) {
